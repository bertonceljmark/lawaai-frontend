...
export default function SharedModal({
  ...
  const handlers = useSwipeable({
    onSwipedLeft: () => {
      if (
        images &&
        Number(photoId) + 1 < Number(images[images.length - 1].id)
      ) {
        changePhotoId(Number(photoId) + 1);
      }
    },
    onSwipedRight: () => {
      if (images && Number(photoId) > Number(images[0].id)) {
        changePhotoId(Number(photoId) - 1);
      }
    },
    trackMouse: true,
        ...
        <div className="absolute inset-0 mx-auto flex max-w-7xl items-center justify-center">
          ...
          {loaded && (
            <div className="relative aspect-[3/2] max-h-full w-full">
              {navigation && (
                <>
                  {images && Number(photoId) > Number(images[0].id) && (
                    <button
              ...
              <div className="absolute top-0 right-0 flex items-center gap-2 p-3 text-white">
                <Link
                  href={currentPhoto ? currentPhoto.url : ""}
                    ...
                    filteredImages.map(({ id, sizes }) => (
                      <motion.button
                        ...
                        className={`${
                          ...
                          images && id === Number(images[0].id)
                            ? "rounded-l-md"
                            : ""
                        } ${
                          images && id === Number(images[0].id) - 1
                            ? "rounded-r-md"
                            : ""
